...
async function carregarPerfil(): Promise<void> {
  ...
  if (dados.foto_perfil_url) (document.getElementById('fotoPerfil') as HTMLElement).outerHTML = `<img src="${html(mediaUrl(dados.foto_perfil_url))}" class="profile-large-avatar" id="fotoPerfil" alt="${html(nome)}">`;
  (document.getElementById('meusLivros') as HTMLDivElement).innerHTML = slider('Meus livros', dados.meus_livros);
}
document.getElementById('botaoAdicionarLivro')?.addEventListener('click', () => location.href = 'adicionar_livro.html');
void carregarPerfil();