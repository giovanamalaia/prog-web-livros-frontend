...
async function carregarConfiguracoes(): Promise<void> {
  ...
  if (d.foto_perfil_url) (document.getElementById('fotoConfiguracoes') as HTMLElement).outerHTML = `<img src="${html(mediaUrl(d.foto_perfil_url))}" class="profile-large-avatar" id="fotoConfiguracoes" alt="${html(d.username)}">`;
