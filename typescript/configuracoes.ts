...
async function carregarConfiguracoes(): Promise<void> {
  ...
  (document.getElementById('username') as HTMLInputElement).value = d.username || '';
  (document.getElementById('first_name') as HTMLInputElement).value = d.first_name || '';
  (document.getElementById('last_name') as HTMLInputElement).value = d.last_name || '';
  (document.getElementById('email') as HTMLInputElement).value = d.email || '';
  (document.getElementById('usuarioConfiguracoes') as HTMLElement).textContent = d.username || 'Perfil';
  ...
  await carregarCidades(
    ...
    document.getElementById('cidade') as HTMLSelectElement,
    d.estado || '',
    d.cidade || '',
  );
}
document.getElementById('formConfiguracoes')?.addEventListener('submit', async (evento) => {
  ...
  if (file instanceof File && !file.name) dados.delete('foto_perfil');
  const resposta = await api('/configuracoes/', { method: 'POST', body: dados });
  mostrarMensagem(
    resposta.message || errorsToText(resposta.errors),
    resposta.status === 'success' ? 'success' : 'error',
  );
