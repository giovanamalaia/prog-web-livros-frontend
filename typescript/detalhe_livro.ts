...
document.addEventListener('click', async evento => { const alvo = evento.target as HTMLElement; if (!livroAtual) return; if (alvo.id === 'detalheDono' && livroAtual.dono_id) { location.href = `perfil_publico.html?id=${livroAtual.dono_id}&next=${encodeURIComponent('detalhe_livro.html?id=' + livroAtual.id)}`; return; } if (alvo.id === 'editar') location.href = `adicionar_livro.html?id=${livroAtual.id}`; if (alvo.id === 'excluir' && confirm('Excluir este livro?')) { await api(`/excluir-livro/${livroAtual.id}/`, { method: 'DELETE' }); location.href = 'perfil.html'; } if (alvo.id === 'tenhoInteresse') { await api(`/livro/${livroAtual.id}/interesse/`, { method: 'POST' }); await carregarDetalhe(); } if (alvo.id === 'removerInteresse') { await api(`/livro/${livroAtual.id}/interesse/excluir/`, { method: 'DELETE' }); await carregarDetalhe(); } });
