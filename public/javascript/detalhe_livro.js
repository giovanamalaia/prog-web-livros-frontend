...
document.addEventListener('click', async (evento) => { const alvo = evento.target; if (!livroAtual)
    return; if (alvo.id === 'detalheDono' && livroAtual.dono_id) {
    location.href = `perfil_publico.html?id=${livroAtual.dono_id}&next=${encodeURIComponent('detalhe_livro.html?id=' + livroAtual.id)}`;
    return;
} if (alvo.id === 'editar')
    location.href = `adicionar_livro.html?id=${livroAtual.id}`; if (alvo.id === 'excluir' && confirm('Excluir este livro?')) {
