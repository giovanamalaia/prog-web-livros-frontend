...
let view = localStorage.getItem(AUTH_KEY) === '1' ? 'home' : 'login';
let notice = '';
let noticeKind = 'info';
...
function cityOptions(uf, selected = '') {
    if (!uf)
        return '<option value="">Selecione o estado primeiro</option>';
...
async function loadCurrentUser() {
    ...
    const response = await api('/configuracoes/');
...
function cover(book, large = false) {
    if (book.capa_url)
        return `<img src="${html(book.capa_url)}" alt="${html(book.titulo)}">`;
...
async function render(nextView) {
    view = nextView;
    await loadCities();
    if (view === 'login') {
    ...
    try {
        if (view === 'home') {
            const response = await api(`/home/${searchQuery ? `?q=${encodeURIComponent(searchQuery)}` : ''}`);
            if (response.status !== 'success')
                throw new Error(response.message || errorsToText(response.errors));
            app.innerHTML = homeView(response.data);
        }
        else if (view === 'favoritos') {
            const response = await api(`/favoritos/${searchQuery ? `?q=${encodeURIComponent(searchQuery)}` : ''}`);
            if (response.status !== 'success')
                throw new Error(response.message || errorsToText(response.errors));
            app.innerHTML = favoritosView(response.data || []);
        }
        else if (view === 'perfil') {
            const response = await api('/perfil/');
            if (response.status !== 'success')
                throw new Error(response.message || errorsToText(response.errors));
            ...
            const response = await api(`/livro/${selectedBookId}/`);
...
app.addEventListener('click', async (event) => {
    ...
    if (viewButton) {
        setNotice('');
        await render(viewButton.dataset.view);
        return;
    }
    if (!actionButton)
        return;
    ...
    if (action === 'new-book') {
        ...
        const response = await api(`/livro/${bookId}/`);
...
app.addEventListener('change', (event) => {
    ...
    if (!field.matches('[data-state-select]'))
        return;
    const cityField = field.closest('form')?.querySelector('[data-city-select]');
    if (cityField)
        cityField.innerHTML = cityOptions(field.value);
});
app.addEventListener('submit', async (event) => {
    ...
    try {
        if (formName === 'search') {
            ...
            if (response.status !== 'success')
                throw new Error(response.message || errorsToText(response.errors) || 'Login inválido.');
            ...
            if (response.status !== 'success')
                throw new Error(response.message || errorsToText(response.errors) || 'Cadastro inválido.');
            localStorage.setItem(AUTH_KEY, '1');
            ...
            if (response.status !== 'success')
                throw new Error(response.message || errorsToText(response.errors) || 'Não foi possível salvar o livro.');
